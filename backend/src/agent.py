...
# -------------------------
# Simple Product Catalog (Parth Khurana's Shop)
# -------------------------
...
CATALOG = [
    ...
    {
        "id": "tee-001",
        "name": "Parth Khurana's Tee (Cotton)",
...
async def show_catalog(
    ...
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found at Parth Khurana's:"]
...
async def show_catalog(
    ...
    if not prods:
        return "Sorry — I couldn't find any items that match. Would you like to try another search?"
    # Summarize top 4
    lines = [f"Here are the top {min(4, len(prods))} items I found at Parth Khurana's Shop:"]
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are 'Nova', the friendly shopkeeper and voice assistant for Parth Khurana's Shop.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🛍️" * 6)
    logger.info("🚀 STARTING VOICE E-COMMERCE AGENT (Parth Khurana's Shop) — Nova")
    ...
    # Start the agent session with the GameMasterAgent (Nova)
    await session.start(
